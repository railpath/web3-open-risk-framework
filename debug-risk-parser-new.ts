/**
 * Debug script for risk parser with new format
 */
...
console.log('Debugging risk parser with new format...')
    ...
    if (fs.statSync(categoryPath).isDirectory()) {
      ...
      if (riskFiles.length > 0) {
        ...
        try {
          ...
          // NEW: Match YAML code blocks
          const yamlMatch = content.match(/```yaml\s*([\s\S]*?)\s*```/)
          if (yamlMatch) {
            console.log(`  YAML code block found`)
            const metadata = yaml.load(yamlMatch[1]) as any
            console.log(`  Metadata: ${JSON.stringify(metadata, null, 2)}`)
          } else {
            console.log(`  No YAML code block found`)
          }
        } catch (error) {
